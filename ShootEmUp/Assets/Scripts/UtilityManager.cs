...
public class UtilityManager {
    ...
    public void FadeOutForUIText(GameObject whosUsingIt,float timeToFadeOut)
    {
        if (startToFade)
        {
            changeColor = whosUsingIt.GetComponent<Text>().color.a;
            if (changeColor >= 0)
            {
                changeColor -= Time.deltaTime / timeToFadeOut;
                whosUsingIt.GetComponent<Text>().color = new Color(255, 255, 255f, changeColor);
            }
            if (changeColor <= 0)
            {
                changeColor = 1f;
